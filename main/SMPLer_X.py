...
class Model(nn.Module):
    def __init__(self, encoder, body_position_net, body_rotation_net, box_net, hand_position_net, hand_roi_net,
                 hand_rotation_net, face_regressor, markers_num=None):
    ...
    def get_coord(self, root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, shape, expr, cam_trans, mode):
        ...
        marker_cam = mesh_cam[:, markers_idxs, :]
        ...
        if mode == 'train' and len(cfg.trainset_3d) == 1 and cfg.trainset_3d[0] == 'AGORA' and len(
            ...
            xm = (marker_cam[:, :, 0].detach() + cam_trans[:, None, 0]) / (
                    marker_cam[:, :, 2].detach() + cam_trans[:, None, 2] + 1e-4) * cfg.focal[0] + cfg.princpt[0]
            ym = (marker_cam[:, :, 1].detach() + cam_trans[:, None, 1]) / (
                    marker_cam[:, :, 2].detach() + cam_trans[:, None, 2] + 1e-4) * cfg.focal[1] + cfg.princpt[1]
            ...
            xm = (marker_cam[:, :, 0] + cam_trans[:, None, 0]) / (marker_cam[:, :, 2] + cam_trans[:, None, 2] + 1e-4) * \
                cfg.focal[0] + cfg.princpt[0]
            ym = (marker_cam[:, :, 1] + cam_trans[:, None, 1]) / (marker_cam[:, :, 2] + cam_trans[:, None, 2] + 1e-4) * \
                cfg.focal[1] + cfg.princpt[1]
        x = x / cfg.input_body_shape[1] * cfg.output_hm_shape[2]
        y = y / cfg.input_body_shape[0] * cfg.output_hm_shape[1]
        joint_proj = torch.stack((x, y), 2)

        xm = xm / cfg.input_body_shape[1] * cfg.output_hm_shape[2]
        ym = ym / cfg.input_body_shape[0] * cfg.output_hm_shape[1]
        marker_proj = torch.stack((xm, ym), 2)

        # root-relative 3D coordinates
        root_cam = joint_cam[:, smpl_x.root_joint_idx, None, :]
        marker_cam = marker_cam - root_cam
        ...
        return joint_proj, joint_cam, joint_cam_wo_ra, mesh_cam, marker_proj, marker_cam
    ...
    def generate_mesh_gt(self, targets, mode):
        ...
        # final output
        joint_proj, joint_cam, joint_cam_wo_ra, mesh_cam, marker_proj, marker_cam = \
            self.get_coord(root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, shape, expr, cam_trans, mode)
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        # 2. Body Regressor
        body_joint_hm, body_joint_img, marker_hm, marker_img = self.body_position_net(img_feat)
        root_pose, body_pose, shape, cam_param, = self.body_regressor(
            body_pose_token, shape_token, cam_token,
            body_joint_img.detach(), marker_img.detach() if marker_img is not None else None)
        ...
        # hand regressor
        _, hand_joint_img, _, _ = self.hand_position_net(hand_feat)  # (2N, J_P, 3)
        ...
        # guess it is bug with this -1
        # lhand_joint_img = torch.cat((cfg.output_hand_hm_shape[2] - 1 - lhand_joint_img[:, :, 0:1], lhand_joint_img[:, :, 1:]), 2)
        lhand_joint_img = torch.cat((cfg.output_hand_hm_shape[2] - lhand_joint_img[:, :, 0:1], lhand_joint_img[:, :, 1:]), 2)
        ...
        # final output
        joint_proj, joint_cam, joint_cam_wo_ra, mesh_cam, marker_proj, marker_cam = self.get_coord(
        ...
        if mode == 'train':
            ...
            if getattr(cfg, 'agora_fix_global_orient_transl', False):
                ...
                if hasattr(cfg, 'smplx_orient_weight'):
                    ...
                    loss['smplx_orient'] = self.param_loss(pose, targets['smplx_pose'], meta_info['smplx_pose_valid'][:, None])[:, :3] * smplx_orient_weight

                loss['smplx_pose'] = self.param_loss(pose, targets['smplx_pose'], meta_info['smplx_pose_valid'][:, None]) * smplx_pose_weight

            else:
                loss['smplx_pose'] = self.param_loss(pose, targets['smplx_pose'], meta_info['smplx_pose_valid'][:, None])[:, 3:] * smplx_pose_weight
            ...
            if self.markers_num > 0:
                smplx_markers_3d_weight = getattr(cfg, 'smplx_markers_3d_weight', 1.0)

                loss['marker_cam'] = self.coord_loss(
                    marker_cam, targets['marker_cam'], meta_info['marker_valid'] * meta_info['is_3D'][:, None, None]
                ) * smplx_markers_3d_weight
            ...
            # if not (meta_info['lhand_bbox_valid'] == 0 + meta_info['rhand_bbox_valid'] == 0).all():
            ...
            if self.markers_num > 0:
                smplx_markers_2d_weight = getattr(cfg, 'smplx_markers_2d_weight', 1.0)
                net_markers_2d_weight = getattr(cfg, 'net_markers_2d_weight', 1.0)

                loss['marker_proj'] = self.coord_loss(
                    marker_proj, targets['marker_img'][:, :, :2],
                    meta_info['marker_trunc']) * smplx_markers_2d_weight
                loss['marker_img'] = self.coord_loss(
                    marker_img, targets['marker_img'],
                    meta_info['marker_trunc'], meta_info['is_3D']) * net_markers_2d_weight
...
def get_model(mode):
    ...
    markers_num = getattr(cfg, 'markers_num', 0)
    print(f'{markers_num=}')
    body_position_net = PositionNet('body', feat_dim=cfg.feat_dim, markers_num=markers_num)
    body_rotation_net = BodyRotationNet(feat_dim=cfg.feat_dim, markers_num=markers_num)
    ...
    model = Model(encoder, body_position_net, body_rotation_net, box_net, hand_position_net, hand_roi_net, hand_rotation_net,
                  face_regressor, markers_num=markers_num)
    return model