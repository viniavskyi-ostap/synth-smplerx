...
def augmentation(img, bbox, data_split, preserve_aspect_ratio=False):
    ...
    # TODO: update rotations with do flip
    do_flip = False

    if preserve_aspect_ratio:
        img, trans, inv_trans, R_aug, S_aug = generate_patch_image_preserve_aspect_ratio(
            img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    else:
        img, trans, inv_trans, R_aug, S_aug = generate_patch_image(
            img, bbox, scale, rot, do_flip, cfg.input_img_shape)
    img = np.clip(img * color_scale[None, None, :], 0, 255)

    return img, trans, inv_trans, rot, do_flip, R_aug, S_aug
...
def generate_patch_image(cvimg, bbox, scale, rot, do_flip, out_shape):
    ...
    trans, R_aug, S_aug = gen_trans_from_patch_cv(
        bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot)
    img_patch = cv2.warpAffine(img, trans, (int(out_shape[1]), int(out_shape[0])), flags=cv2.INTER_LINEAR)
    img_patch = img_patch.astype(np.float32)
    inv_trans, _, _ = gen_trans_from_patch_cv(
        bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot, inv=True)

    return img_patch, trans, inv_trans, R_aug, S_aug


def generate_patch_image_preserve_aspect_ratio(cvimg, bbox, scale, rot, do_flip, out_shape):
    img = cvimg.copy()
    img_height, img_width, img_channels = img.shape
...
def gen_trans_from_patch_cv(c_x, c_y, src_width, src_height, dst_width, dst_height, scale, rot, inv=False):
    ...
    # decompose affine matrix A = SR, where S is scale + translation, R is rotation
    A = np.concatenate([trans, np.array([[0, 0, 1]])], axis=0)

    cos_rot, sin_rot = np.cos(np.deg2rad(rot)), np.sin(np.deg2rad(rot))
    R = np.array([
        [cos_rot, sin_rot, 0],
        [-sin_rot, cos_rot, 0],
...
def process_db_coord(joint_img, joint_cam, joint_valid, do_flip, img_shape, flip_pairs, img2bb_trans, rot,
                     ...
                     markers_img=None, markers_cam=None, markers_valid=None, markers_flip_pairs=None):
    ...
    joint_trunc = joint_valid * (
            ...
            (joint_img_original[:, 2] > 0) * (joint_img[:, 2] >= 0) * (joint_img[:, 2] < cfg.output_hm_shape[0])
    ...
    joints_root = joint_cam_wo_ra[smpl_x.root_joint_idx, None, :]  # root-relative
    joint_cam_wo_ra = joint_cam_wo_ra - joints_root
    ...
    if markers_img is not None:
        markers_img, markers_cam, markers_valid = markers_img.copy(), markers_cam.copy(), markers_valid.copy()
        markers_img_original = markers_img.copy()
        if do_flip:
            markers_cam[:, 0] = -markers_cam[:, 0]
            markers_img[:, 0] = img_shape[1] - 1 - markers_img[:, 0]
            for pair in markers_flip_pairs:
                markers_img[pair[0], :], markers_img[pair[1], :] = \
                    markers_img[pair[1], :].copy(), markers_img[pair[0], :].copy()
                markers_cam[pair[0], :], markers_cam[pair[1], :] = \
                    markers_cam[pair[1], :].copy(), markers_cam[pair[0], :].copy()
                markers_valid[pair[0], :], markers_valid[pair[1], :] = \
                    markers_valid[pair[1], :].copy(), markers_valid[pair[0], :].copy()

        markers_cam = np.dot(rot_aug_mat, markers_cam.transpose(1, 0)).transpose(1, 0)

        # affine transformation
        markers_img_xy1 = np.concatenate((markers_img[:, :2], np.ones_like(markers_img[:, :1])), 1)
        markers_img[:, :2] = np.dot(img2bb_trans, markers_img_xy1.transpose(1, 0)).transpose(1, 0)
        markers_img[:, 0] = markers_img[:, 0] / cfg.input_img_shape[1] * cfg.output_hm_shape[2]
        markers_img[:, 1] = markers_img[:, 1] / cfg.input_img_shape[0] * cfg.output_hm_shape[1]

        markers_trunc = markers_valid * (
                (markers_img_original[:, 0] > 0) * (markers_img[:, 0] >= 0) * (
                markers_img[:, 0] < cfg.output_hm_shape[2]) *
                (markers_img_original[:, 1] > 0) * (markers_img[:, 1] >= 0) * (
                        markers_img[:, 1] < cfg.output_hm_shape[1]) *
                (markers_img_original[:, 2] > 0) * (markers_img[:, 2] >= 0) * (
                        markers_img[:, 2] < cfg.output_hm_shape[0])
        ).reshape(-1, 1).astype(np.float32)

        # root-alignment, for joint_cam input wo ra
        markers_cam = markers_cam - joints_root  # root-relative
    else:
        markers_cam, markers_trunc = None, None

    return joint_img, joint_cam_wo_ra, joint_cam_ra, joint_valid, joint_trunc, \
        markers_img, markers_cam, markers_valid, markers_trunc
