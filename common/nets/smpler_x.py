...
class PositionNet(nn.Module):
    def __init__(self, part, feat_dim=768, markers_num=None):
        super(PositionNet, self).__init__()
        self.markers_num = markers_num
        if part == 'body':
            self.joint_num = len(smpl_x.pos_joint_part['body'])
            if self.markers_num is not None:
                self.joint_num += self.markers_num
    ...
    def forward(self, img_feat):
        ...
        if self.markers_num is not None:
            joint_hm, marker_hm = \
                joint_hm[:, :self.joint_num - self.markers_num], joint_hm[:, self.joint_num - self.markers_num:]
            joint_coord, marker_coord = \
                joint_coord[:, :self.joint_num - self.markers_num], joint_coord[:, self.joint_num - self.markers_num:]
        else:
            marker_hm, marker_coord = None, None

        return joint_hm, joint_coord, marker_hm, marker_coord
...
class BodyRotationNet(nn.Module):
    def __init__(self, feat_dim=768, markers_num=None):
        super(BodyRotationNet, self).__init__()
        self.markers_num = markers_num if markers_num is not None else 0

        self.joint_num = len(smpl_x.pos_joint_part['body'])
        self.body_conv = make_linear_layers([feat_dim, 512], relu_final=False)
        self.root_pose_out = make_linear_layers([
            self.joint_num * (512 + 3) + self.markers_num * 3, 6], relu_final=False)
        self.body_pose_out = make_linear_layers(
            [self.joint_num * (512 + 3) + self.markers_num * 3, (len(smpl_x.orig_joint_part['body']) - 1) * 6],
            relu_final=False)  # without root
        self.shape_out = make_linear_layers([feat_dim + self.markers_num * 3, smpl_x.shape_param_dim], relu_final=False)
        self.cam_out = make_linear_layers([feat_dim, 3], relu_final=False)
        self.feat_dim = feat_dim

    def forward(self, body_pose_token, shape_token, cam_token, body_joint_img, body_marker_img=None):
        batch_size = body_pose_token.shape[0]

        if body_marker_img is not None and self.markers_num > 0:
            body_marker_img = body_marker_img.view(batch_size, -1)  # [B, N=64, 3] -> [B, 3*N=192]
        else:
            body_marker_img = body_pose_token.new_empty(batch_size, 0)  # [B, 0]

        # shape parameter
        shape_param = self.shape_out(torch.cat([shape_token, body_marker_img], dim=1))
        ...
        body_pose_token = torch.cat([body_pose_token.view(batch_size, -1), body_marker_img], dim=1)
        root_pose = self.root_pose_out(body_pose_token)
        body_pose = self.body_pose_out(body_pose_token)
...
class HandRoI(nn.Module):
    ...
    def forward(self, img_feat, lhand_bbox, rhand_bbox):
        ...
        assert (cfg.output_hm_shape[1] * self.upscale, cfg.output_hm_shape[2] * self.upscale) == (
            img_feat.shape[2], img_feat.shape[3])
        lhand_img_feat = roi_align(img_feat, lhand_bbox_roi, (cfg.output_hand_hm_shape[1], cfg.output_hand_hm_shape[2]),
                                   1.0, 0, 'avg', False)
        ...
        rhand_img_feat = roi_align(img_feat, rhand_bbox_roi, (cfg.output_hand_hm_shape[1], cfg.output_hand_hm_shape[2]),
                                   1.0, 0, 'avg', False)
